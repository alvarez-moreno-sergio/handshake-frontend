...
export async function handleIncomingFile(
    ...
    if (msg.type === "file_metadata") {
        ...
        return finalizeTransfer(msg.fileId);
    }
}
...
function storeChunk(
    ...
    let storedChunks: Map<number, ArrayBuffer> | undefined = incomingFiles.get(fileId);
    if (!storedChunks) {
        storedChunks = new Map<number, ArrayBuffer>()
        incomingFiles.set(fileId, storedChunks);
    }
    storedChunks.set(chunkIndex, chunk);
}
